...
load("3-data_analysis/ipop_metabolomics_data_preparation/peak/object_corss_section")

dir.create("3-data_analysis/ipop_feature_analysis/loess_fit",
           recursive = TRUE)
setwd("3-data_analysis/ipop_feature_analysis/loess_fit")

object_corss_section 

dim(object_corss_section)
...
expression_data <-
  extract_expression_data(object_corss_section)
sample_info <-
  extract_sample_info(object_corss_section)
variable_info <-
  extract_variable_info(object_corss_section)
...
for (i in seq_along(variable_info$variable_id)) {
  ...
  prediction_value =
    predict(ls_reg,
            newdata = data.frame(adjusted_age = adjusted_age))
...
object_corss_section_loess@expression_data <-
  new_expression_data

save(object_corss_section_loess, file = "object_corss_section_loess")
