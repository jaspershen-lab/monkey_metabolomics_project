check4fmsea <- function(object_list) {
  ...
  object_list <-
    ...
    purrr::map(function(x) {
      variable_info <-
        massdataset::extract_variable_info(x)
      sample_info <-
        massdataset::extract_sample_info(x)
      if (all(colnames(variable_info) != "mean_intensity")) {
        if (any(unique(sample_info$class) == "QC")) {
          sample_id <-
            sample_info$sample_id[sample_info$class == "QC"]
        } else{
          sample_id <-
            sample_info$sample_id[sample_info$class == "Subject"]
        }
        x <-
          x %>%
          massdataset::mutate_mean_intensity(object = x,
